...
const リスト = ({ workspace, taskList, userList }) => {
  ...
  const columns = [
    {
      ...
      dataIndex: "workspace",
      key: "workspace",
      // render: (workspace) => <span>{workspace.title}</span>,
  ...
  useEffect(() => {
    (async () => {
      ...
      setListTask(result?.filter(item => item.workspace == workspace.id))
          ...
          >
            ...
            <div className="task-count-list">
              <span className="task-count-todo">
                {taskCount(listTask, "To Do")}
              </span>
              <span className="task-count-doing">
                {taskCount(listTask, "In Progress")}
              </span>
              <span className="task-count-done">
                {taskCount(listTask, "Done")}
              </span>
            </div>
          </div>
