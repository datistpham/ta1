...
export const EventModal2 = ({setEdit, _id, setChange}) => {
  ...
  async function handleSubmit(e) {
    ...
    const result = await updateTaskApi(calendarEvent);
    setSelectedEvent()
    setChange(prev=> !prev)
    setEdit(false)
    console.log(result);
      ...
      >
        <header className="bg-gray-100 px-4 py-2 flex justify-between items-center">
          ...
          <div>
            {selectedEvent && (
              <span
                ...
                onClick={async () => {
                  ...
                  const result= await deleteTaskApi({taskId: _id})
                  setSelectedEvent()
                  setChange(prev=> !prev)
                  setEdit(false)
                }}
            ...
            <button onClick={() => {setEdit(false); setSelectedEvent()}}>
              <span className="material-icons-outlined text-gray-400">
                close
              </span>
            </button>
        ...
        <div className="p-3">
          <div className="grid grid-cols-1/3 items-end gap-y-7">
            ...
            <p className="text-red-400">
              {deadline !== null ? (
                Math.floor((new Date(deadline).getTime() - daySelected.valueOf()) /
                  (1000 * 60 * 60 * 24)) +
                `d`
              ) : (
                <></>
